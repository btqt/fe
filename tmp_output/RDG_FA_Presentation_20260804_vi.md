<!-- Slide 1 -->
# Thiết kế Kiến trúc Chức năng cho RemoteDiag (RDG) trong quá trình tách EP
...
2. Xác định vấn đề
3. Đề xuất giải pháp
4. Thiết kế chi tiết cho giải pháp được chọn
...
<!-- Slide 3 -->
# 1. Tổng quan — DCM và module RDG trong dự án Toyota 24LM
...
*Chi tiết tại Phụ lục 1 (Sơ đồ tổng quan hệ thống)*

---

<!-- Slide 4 -->
# 2. Xác định vấn đề — Bối cảnh xuất phát
...
*Chi tiết tại Phụ lục 2 (Yêu cầu chức năng) và Phụ lục 3 (Ràng buộc)*

---

<!-- Slide 5 -->
# 2. Xác định vấn đề — Các thuộc tính chất lượng trọng tâm

| Quality Attribute | Độ ưu tiên | Mô tả |
| --- | --- | --- |
| Security | **Cao** | Compromise giới hạn trong EP; EP mạo danh DIAG client → discard; filter SID reprogramming (NFR-01/02/03) |
| Performance | Cao | Không truyền gói ~180MB qua SPI hai lần; cân nhắc pointer-based transfer (NFR-04) |
| Reliability | Trung bình | Fail-safe: POLLHUP detect, reconnect, reset nếu abnormal restart ≥4 lần/480s (NFR-05) |
| Availability | Trung bình | Resource Quotas: EP Container không được làm đói IP Container (NFR-06) |
| Maintainability | Trung bình | Không phải định nghĩa/hiện thực IPC mới cho từng module EP (NFR-07) |
| Schedule | Cao | Bring-up 7/17 → 7/24 (triển khai), 7/31 (kiểm chứng) — rất gấp (NFR-08, C-06) |
...
<!-- Slide 6 -->
# 3. Đề xuất giải pháp — Alt A: Tách RDG thành nhiều app ở hai bên

Chia RDG thành nhiều app theo Server/MM SID path — phần cần CAN access ở IP, phần giao tiếp Center/EP ở EP (design 5/19, cuộc họp Nagoya).
...
<!-- Slide 7 -->
# 3. Đề xuất giải pháp — Alt B: RDG ở IP + IPC socket per-module
...
<!-- Slide 8 -->
# 3. Đề xuất giải pháp — Alt C: RDG ở IP + RemoteDiag Proxy transparent
...
<!-- Slide 9 -->
# 3. Đề xuất giải pháp — Alt D: Generic Container Gateway/Message Broker *(đề xuất bổ sung)*
...
<!-- Slide 10 -->
# 3. Đề xuất giải pháp — So sánh & Lựa chọn

| Tiêu chí | Độ ưu tiên | Alt A (tách RDG) | Alt B (IPC per-module) | **Alt C (RDG + RDP)** | Alt D (Gateway)* |
| --- | --- | --- | --- | --- | --- |
| Security | Cao | ✗ Logic diag ở EP | ✓ | ✓ Filter trọn trusted zone | ✓ |
| Maintainability | Trung bình | ✗ Chẻ đôi codebase | ✗ ≥10 module sửa code | ✓ **Zero change** ở EP | ~ Platform mới |
| Performance | Cao | ~ Đồng bộ state tốn kém | ✓ Trực tiếp | ~ +1 hop socket | ✗ +2 hop |
| Reliability | Trung bình | ✗ Nhiều failure mode | ✗ N điểm fail-safe | ~ RDP SPOF (có pattern sẵn) | ✗ Broker SPOF |
| Schedule | Cao | ✗ Thay đổi lớn nhất | ✗ Effort × N | ✓ Tái dùng khung sẵn có | ✗ Vượt scope |
...
🏆 **Được chọn: Alt C — RDG toàn phần ở Internal Container + RemoteDiag Proxy transparent ở EP** (AD-RDG-01)

*\* Alt D chưa qua thảo luận HQ–LGEDV; giữ làm hướng dài hạn.*

---

<!-- Slide 11 -->
# 4. Thiết kế chi tiết — Sơ đồ tĩnh (Alt C)
...
**Chú giải:** ô mới = thành phần mới (RDP, SocketServer/SID Filter trong RDG); còn lại = thành phần hiện hữu; nét đứt = lifecycle event.
...
<!-- Slide 12 -->
# 4. Thiết kế chi tiết — Cấu trúc chức năng nội bộ RDG
...
<!-- Slide 13 -->
# 4. Thiết kế chi tiết — Sơ đồ động: SID36 DCM update (AD-RDG-05)
...
**Lý do chọn:** giữ policy OBC (C-01, không thỏa thuận lại OEM); tránh SPI double-transfer cho 180MB (C-02); tái dùng kênh RDG↔RDP. Option bị loại: #1 (bỏ qua OBC — giữ làm fallback), #3/#4 (IPC/Proxy mới), #5 (qua MCU — heavy SPI).

*Sequence UDS Tx/Rx tổng quát, Lifecycle và Grade switching tại Phụ lục 4–6*
...
Kiểm chứng kiến trúc theo 5 nhóm test (mốc TMCDCMLM-157: triển khai 7/24, kiểm chứng 7/31):

| # | Nhóm test | Nội dung | Yêu cầu |
| --- | --- | --- | --- |
...
| V-3 | Performance SID36 | Đo throughput chunk 512KB trên Option #2 vs time-budget 180MB; thất bại → fallback Option #1 | FR-02, NFR-04 |
...
flowchart LR
    ...
    V3 -->|thất bại| FB["Fallback Option #1<br/>(thảo luận OEM)"]
```
...
| Quality Attribute | Độ ưu tiên | Kết quả với Alt C | Đạt |
| --- | --- | --- | --- |
| Security | Cao | RDG + SID Filter trọn trusted zone; ingress control tại IP; EP compromise không lan sang CAN | ✅ |
| Maintainability | Trung bình | EP modules zero change; một điểm hội tụ IPC | ✅ |
| Performance | Cao | Đường 180MB ở lại trong AP; +1 hop socket chấp nhận được; pointer-based transfer còn open | ⚠️ (R-01) |
| Reliability | Trung bình | RDP SPOF — mitigate bằng fail-safe pattern chuẩn, xử lý sau bring-up | ⚠️ (R-02) |
| Schedule | Cao | Tái dùng khung ManagerProxy sẵn có, đáp ứng mốc 7/17–7/31 | ✅ |
...
<!-- Slide 17 -->
# Phụ lục 1 — Sơ đồ tổng quan hệ thống
...
<!-- Slide 20 -->
# Phụ lục 4 — Sequence: UDS Tx/Rx tổng quát
...
sequenceDiagram
    ...
    participant RDP as [EP] RemoteDiag Proxy
    Note over RDG,MCU: Đường truyền Tx
    RDG->>OBC: SendUdsData()
    OBC->>ICM: SendDataToMcu()
    ICM->>MCU: SPI
    Note over MCU,RDG: Đường nhận Rx
    MCU->>CM: CAN/UDS data
...
<!-- Slide 21 -->
# Phụ lục 5 — Sequence: Lifecycle / BOOT_COMPLETE (AD-RDG-06)
...
sequenceDiagram
    ...
    Note over AMI,RDG: Trình tự boot trong IP
...
<!-- Slide 22 -->
# Phụ lục 6 — Sequence: Grade switching DCM5.2 (AD-RDG-07)
...
<!-- Slide 23 -->
# Phụ lục 7 — Tổng hợp các Architectural Decisions

| AD | Quyết định | Lý do chính |
| --- | --- | --- |
...
<!-- Slide 24 -->
# Phụ lục 8 — Open Issues / Rủi ro còn lại

| # | Loại | Mô tả | Trạng thái |
| --- | --- | --- | --- |
| R-01 | Performance | Binder/socket copy cho chunk 512KB × ~360 lần (180MB) trên SID36 Option #2 | Đang mở — pointer-based transfer; fallback Option #1 |
| R-02 | Reliability | RDP crash → mất toàn bộ luồng EP↔RDG | Đang mở — khuyến nghị ServiceMonitor POLLHUP |
| R-03 | Security | Kênh socket EP↔IP chưa có encryption/authentication | Đang mở |
| R-04 | Functional | "stop all operation" (grade disable) chưa spec đầy đủ | Đang trao đổi với TMC (MPWSPEC-44) |
